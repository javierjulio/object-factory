<cfcomponent displayname="Factory" hint="Handles dependency injection on transient and singleton objects" output="false">
	...
	<cffunction name="setDOM" access="private" output="false" returntype="void">
		<cfargument name="dom" type="struct" required="yes">
		<cfset variables.dom = arguments.dom>
	</cffunction>
	...
	<cffunction name="getObject" access="public" output="false" returntype="any">
		...
		<cfargument name="beanRef" type="xml" required="no">
		...
		<cfelse>
			...
			<!--- loop through all results and store created properties in temp struct --->
			...
			<cfif structKeyExists(dom[arguments.id].xmlAttributes, "factory") AND structKeyExists(dom[arguments.id].xmlAttributes, "method")>
				...
				<!--- if we get this far then requested method exists, call it, and pass any constuctor arguments --->
			...
			<cfelse>
				<cfset xml = xmlSearch(dom[arguments.id], "//bean[@name='#arguments.id#']/property")>
			</cfif>
	...
	<cffunction name="determinePath" access="private" output="false" returntype="void">
		...
		<cfelseif fileExists(arguments.xmlPath)>
			<!--- otherwise its just a single xml file --->
